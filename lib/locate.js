...
export function locate(text, key) {
  ...
  // Laravel-Lang and every other project that keys by the English sentence puts
  // dots inside the key: `Load :perPage More.` splits into nonsense. So the
  // whole key is tried first, then the same without its namespace, before
  // falling back to walking the nesting.
  for (const candidate of [key, key.replace(/^[^.]+\./, '')]) {
    for (let i = 0; i < lines.length; i++) {
      const column = columnOf(lines[i], candidate);
